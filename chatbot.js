...
// OpenAI API Key (NOTE: This is insecure for production)
const OPENAI_API_KEY = "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";
...
// Send user message when the "Send" button is clicked or "Enter" is pressed
...
function updateLastBotMessage(message) {
    const messages = chatbotMessages.querySelectorAll("div");
    const lastMessage = messages[messages.length - 1];
    if (lastMessage.textContent.startsWith("MelBot:")) {
        lastMessage.textContent = `MelBot: ${message}`;
    }
}
...
document.addEventListener("mousemove", (e) => {
    if (isDragging) {
        ...
        // Update chatbot image position
        chatbotToggle.style.left = `${e.clientX - offsetX}px`;
        chatbotToggle.style.top = `${e.clientY - offsetY}px`;
    }
});
